...
// Contractions live in their own lexicon and this script does not derive from
// them — build/gen-pls-contractions.py does. But their spellings were merged
// into the hand-maintained PLS on 13 Aug, so without this file the drift report
// calls all 102 of them orphans and the handful of REAL orphans is lost in the
// noise. Read for the orphan check only; nothing here is derived or written.
const CONTRACTIONS = join(root, 'data/euspell_lexicon_contractions.csv');
...
// dict/euspell_tts.pls is NO LONGER WRITTEN. As of 11 Aug 2026 it is maintained
// by hand and is the source of truth for pronunciation: it carries corrections
// that exist nowhere upstream — voicing (absolv æbˈzɑlv, abuzable əˈbjuzəbəl),
// stress (abstractz æbˈstrækts), and entries with no lexicon row behind them at
// all. Regenerating it would silently discard all of that, so the write is gone
// rather than guarded. What `pls` still holds is the derived text, kept for the
// drift report below.
const derived = new Map([...entries.values()].map((e) => [e.grapheme, e]));
const shipped = readFileSync(OUT, 'utf8');
const LEXEME_RE = /<grapheme>([^<]+)<\/grapheme><phoneme>([^<]+)<\/phoneme>/g;
const hand = new Map();
for (const m of shipped.matchAll(LEXEME_RE)) hand.set(m[1], m[2]);

// The Arpabet twin is transliterated from the hand-maintained IPA file, not from
// `entries`. Deriving it from the generator would reintroduce exactly the values
// the hand edits removed, and the two files would disagree.
const unmappable = [];
const arpaBody = [...hand.keys()]
  .sort((a, b) => a.localeCompare(b))
  .map((grapheme) => {
    const arpa = toArpabet(hand.get(grapheme));
    const word = derived.get(grapheme)?.word ?? grapheme;
    if (!arpa) { unmappable.push(`${grapheme} (${word}): ${hand.get(grapheme)}`); return null; }
    return lexeme({ grapheme, ipa: arpa, word });
  })
  .filter(Boolean)
  .join('\n');

writeFileSync(OUT_ARPA, `<?xml version="1.0" encoding="UTF-8"?>
<!-- GENERATED by build/gen-pls.js by transliterating dict/euspell_tts.pls, which
     is maintained by hand. Do not hand-edit this file; edit the IPA one and rerun. -->
...
console.log(`[gen-pls] ${OUT} is hand-maintained and was NOT written (${hand.size} lexemes read)`);
console.log(`[gen-pls] wrote ${OUT_ARPA} (${hand.size - unmappable.length} lexemes, `
  + `${unmappable.length} refused as unmappable), transliterated from it`);
for (const u of unmappable.slice(0, 12)) console.log(`[gen-pls]     unmappable: ${u}`);

// Drift report. The lexicon still moves — words get respelled, new ones are
// added — and the hand-maintained file has to be told. Nothing here is applied
// automatically; it is a list of edits for a person to make.
const addable = [...derived.keys()].filter((g) => !hand.has(g));
const differs = [...hand.keys()].filter((g) => derived.has(g) && derived.get(g).ipa !== hand.get(g));
// A grapheme the generator cannot derive is not necessarily an orphan. Most are
// traditional spellings the reform retains for one sense — `abstracts` beside
// `abstractz` — which are legitimate entries the derivation simply has no second
// reading for. Only a grapheme no lexicon row mentions at all is unaccounted for.
const known = new Set();
for (const { list } of spellings.values()) for (const s of list) known.add(s);
// Contraction spellings are known to the project even though this script never
// derives them; see CONTRACTIONS above.
for (const raw of readFileSync(CONTRACTIONS, 'utf8').split('\n').slice(1)) {
  const col = raw.trim().split(',');
  if (col.length < 4) continue;
  for (const s of col[3].split('|')) if (s && s !== '[]') known.add(s);
}
const orphan = [...hand.keys()].filter((g) => !derived.has(g) && !known.has(g));
const undrivable = [...hand.keys()].filter((g) => !derived.has(g) && known.has(g));
console.log(`[gen-pls]   vs the lexicon: ${addable.length} derivable spelling(s) it lacks, `
  + `${undrivable.length} stated by hand that cannot be derived, ${orphan.length} in no lexicon row, `
  + `${differs.length} where the hand reading differs`);
for (const g of addable.slice(0, 15)) {
  console.log(`[gen-pls]       missing: ${g} (${derived.get(g).word}) — would be ${derived.get(g).ipa}`);
}
if (addable.length > 15) console.log(`[gen-pls]       … and ${addable.length - 15} more`);
for (const g of orphan) console.log(`[gen-pls]       in no lexicon row: ${g}`);
...
if (stillOpen.length) {
  console.log(`[gen-pls]   ${stillOpen.length} spelling(s) with more spellings than readings and no override:`);
  for (const u of stillOpen) console.log(`[gen-pls]       ${u.grapheme} (${u.word})`);
}
