...
| **iOS** 17+ | not available yet | — |

Those first three are what a release contains; the release workflow builds and
attaches them automatically. It has no Android job, so **no APK is published**
and the preview has to be built from source. iOS has not been submitted anywhere.
...
**Eupub is not available for iPhone or iPad.** The port is in progress and has
not been submitted to the App Store, so there is nothing to search for and no way
to install it without a developer setup.

With one: open `ios/Eupub/Eupub.xcodeproj` in Xcode, set a signing **Team**, and
**Run** onto a device or simulator. That is a development build, not a
distributable one.
...
| iOS: cannot find Eupub in the App Store | It is not there. The port is in progress and has not been submitted, so no search or store region will find it |
