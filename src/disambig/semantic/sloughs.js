 ...
 *   sluffs — /slʌfs/ plural noun: cast-off layers of dead skin
 *   sluffz — /slʌfs/ the verb ("the mucosa sloughs off")
 * Corpus: disambig/sloughs.txt
 *
 * The pronunciation-critical axis is the three-way sense (/slaʊ/ vs /sluː/ vs
 * /slʌf/); the noun/verb split within the shed sense is homophonic — sluffs and
 * sluffz are both /slʌfs/ — so a POS slip there is only orthographic. Sense is
...
const NOUN = { shed: 'sluffs', mire: 'slouhs', backwater: 'sluhs' };

/**
 * @param {Token[]} tokens
 * @param {number} idx
 * @returns {'slouhs' | 'sluhs' | 'sluffs' | 'sluffz'}
 */
export function disambiguate_sloughs(tokens, idx) {
  ...
  if (sense === 'shed') return verb ? 'sluffz' : 'sluffs';
  return NOUN[sense];
}
